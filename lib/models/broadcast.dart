...
final broadcastServiceListProvider =
    NotifierProvider.autoDispose<
      BonsoirBroadcastServiceListNotifier,
      List<BonsoirService>
    >(BonsoirBroadcastServiceListNotifier.new);

/// A model that allows to control the services to broadcast.
class BonsoirBroadcastServiceListNotifier
...
final broadcastServiceStateProvider = AsyncNotifierProvider.autoDispose
    .family<
      BonsoirBroadcastServiceStateNotifier,
      BonsoirBroadcastState,
      BonsoirService
    >(BonsoirBroadcastServiceStateNotifier.new);

/// A model that allows report the broadcast state of a service.
class BonsoirBroadcastServiceStateNotifier
    extends AsyncNotifier<BonsoirBroadcastState> {
  /// The Bonsoir service.
  ...
  void _onEventOccurred(BonsoirBroadcastEvent event) {
    ...
    switch (event) {
      case BonsoirBroadcastStartedEvent():
        state = AsyncData(BonsoirBroadcastStartedState(service: event.service));
        break;
      case BonsoirBroadcastStoppedEvent():
        state = AsyncData(BonsoirBroadcastStoppedState(service: event.service));
        break;
      default:
        break;
    }
...
sealed class BonsoirBroadcastState {
  ...
  const BonsoirBroadcastState({required this.service});
}
...
class BonsoirBroadcastReadyState extends BonsoirBroadcastState {
  ...
  const BonsoirBroadcastReadyState({required super.service});
}
...
class BonsoirBroadcastStartedState extends BonsoirBroadcastState {
  ...
  const BonsoirBroadcastStartedState({required super.service});
}
...
class BonsoirBroadcastStoppedState extends BonsoirBroadcastState {
  ...
  const BonsoirBroadcastStoppedState({required super.service});
}
