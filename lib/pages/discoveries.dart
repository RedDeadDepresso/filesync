...
class DiscoveriesPageWidget extends ConsumerWidget {
  ...
  const DiscoveriesPageWidget({super.key});
    ...
    return Center(
      child: ListView(
        ...
        children: [
          ...
          for (String type in discoveries) _DiscoveryTypeWidget(type: type),
        ],
      ),
    );
  }
...
class _DiscoveryTypeWidgetState extends ConsumerState<_DiscoveryTypeWidget>
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        _TypeHeaderWidget(type: widget.type, state: discoveryState.value),
        if (discoveryState.isLoading)
          ...
          Card(
            child: ListTile(
              ...
              title: Text('Searching for services of type "${widget.type}"...'),
            ),
          )
        else
...
class _TypeHeaderWidget extends ConsumerWidget {
  ...
  const _TypeHeaderWidget({required this.type, this.state});

  @override
  Widget build(BuildContext context, WidgetRef ref) => ListTile(
    leading: const Icon(Icons.chevron_right),
    title: Text(type, style: Theme.of(context).textTheme.titleLarge),
    subtitle: state == null
        ? null
        : Text('Discovery state : $discoveryStateText'),
    trailing: TextButton(
      child: const Text('Stop'),
      onPressed: () =>
          ref.read(discoveryTypeListProvider.notifier).remove(type),
    ),
  );
