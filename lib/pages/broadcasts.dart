...
class BroadcastsPageWidget extends ConsumerWidget {
  ...
  const BroadcastsPageWidget({super.key});
      ...
      child: ListView(
        ...
        children: [
          ...
          for (BonsoirService broadcast in broadcasts)
            _BroadcastServiceWidget(service: broadcast),
        ],
      ),
    );
  }
...
class _BroadcastServiceWidget extends ConsumerStatefulWidget {
  ...
  const _BroadcastServiceWidget({required this.service});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _BroadcastServiceWidgetState();
}

/// The broadcast service widget state.
class _BroadcastServiceWidgetState
    extends ConsumerState<_BroadcastServiceWidget>
    with AutomaticKeepAliveClientMixin<_BroadcastServiceWidget> {
  @override
  ...
  void initState() {
    super.initState();
    ref.listenManual(broadcastServiceStateProvider(widget.service), (_, next) {
      switch (next.value) {
  ...
  Widget build(BuildContext context) {
    super.build(context);
    AsyncValue<BonsoirBroadcastState> broadcastState = ref.watch(
        ...
        ServiceWidget(
          ...
          trailing: broadcastState.isLoading
              ...
              : TextButton(
                  ...
                  onPressed: () => ref
                      .read(broadcastServiceListProvider.notifier)
            ...
            child: Text.rich(
              TextSpan(
                children: [
                  ...
                  TextSpan(text: ' Broadcast state : $broadcastStateText'),
                ],
              ),
